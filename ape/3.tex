  ...
  Nous allons tout d'abord appliquer l'algorithme de Dijkstra sur le graphe suivant pour trouver le chemin de poids minimum entre A et B.
		...
		\begin{algorithm}[h!]
			...
			\begin{algorithmic}
				...
				\COMMENT {Initialisation du graphe, on met les distances à $+\inf$ sauf pour la source}
				\FOREACH	{$v$ in $vertices$}
					\IF{$v\ ==\ source$}
						\STATE distance[$v$] \leftarrow $0$
					\ELSE
						\STATE distance[$v$] \leftarrow $+\inf$
					\ENDIF
					\STATE predecessor[$v$] \leftarrow \textbf{null}	
				\ENDFOR
				
				\COMMENT {Opérations sur les distances des nœuds}
				
				\FOREACH {$u \neq$ source in $vertices$}
					\FOREACH {edge($u$,$v$) with weight $w$ in $edges$}
				...
				\COMMENT {Vérification de la présence de cycles de poids négatif}
